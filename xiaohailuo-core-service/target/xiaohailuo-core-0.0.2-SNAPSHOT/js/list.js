...
function initRecords(data) {
    ...
    for (var j = 0; j < recordListData.value.length; j++) {
        var audioPath = recordListData.value[j].recordFile;
        if (audioPath.startWith("http") || audioPath.startWith("HTTP")) {
            recordListData.value[j].audioType = 'audio';
        } else {
            recordListData.value[j].audioType = 'wechat';
        }
    }
