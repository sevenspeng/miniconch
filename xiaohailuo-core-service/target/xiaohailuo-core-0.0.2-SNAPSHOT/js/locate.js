...
AMap.event.addListener(geolocation, 'complete', onLocateSuccess);//返回定位信息
AMap.event.addListener(geolocation, 'error', onLocateFailed);      //返回定位出错信息
...
//定位失败之后进行城市定位
function onLocateFailed(result) {
    document.getElementById('locating').style.display = 'none';
...
content.innerHTML = "<img src='./images/mark_bs.png'>";
...
var onMapMoveEnd = function () {
   ...
   // loadRecordMarkOfScreen();//初始化屏幕范围内的录音点
};
