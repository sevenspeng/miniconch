/**
 * Created by JINGDAYAN962 on 2017-03-23.
 */
(function () {
    $.fn.weixinAudio = function (options) {
        ...
        Plugin.prototype = {
            ...
            play: function () {
                var self = this;
                var audioPath = self.Audio.src;
                if (self.currentState === "play") {
                    self.pause();
                    return;
                }
                else if (audioPath.lastIndexOf('.wav') > 0 || audioPath.lastIndexOf('.mp3') > 0) {
                    self.Audio.play();
                }
                else {
                    //下载录音到本地
                    this.voiceId = audioPath.substr(audioPath.lastIndexOf('/') + 1);
                    wx.downloadVoice({
                        serverId: this.voiceId,
                        success: function (res) {
            ...
            pause: function () {
                ...
                if (self.voiceId != '') {
                    wx.pauseVoice({
                        localId: this.voiceId// 需要停止的音频的本地ID，由stopRecord接口获得
                    });
                } else {
                    self.Audio.pause();
                }
